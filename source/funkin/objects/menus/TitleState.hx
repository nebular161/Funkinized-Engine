...
import funkin.settings.PlayerSettings;
import funkin.backend.system.Highscore;
import funkin.game.cutscenes.VideoState;
import funkin.game.Conductor;
import funkin.game.cutscenes.CutsceneAnimTestState;
...
class TitleState extends MusicBeatState {
	...
	override public function create():Void {
		...
		FlxG.save.bind('funkinized-engine', 'greyson430');
	...
	function startIntro() {
		...
		logoBl.frames = Paths.getSparrowAtlas('menu objects/title menu/logoBumpin');
		...
		gfDance.frames = Paths.getSparrowAtlas('menu objects/title menu/gfDanceTitle');
		...
		titleText.frames = Paths.getSparrowAtlas('menu objects/title menu/titleEnter');
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('menu objects/title menu/newgrounds_logo'));
		ngSpr.visible = false;
			...
			for (i in lastBeat...curBeat) {
				switch (i + 1) {
					...
					case 4:
						deleteCoolText();
					case 5:
						createCoolText(['Asscociated', 'with']);
					case 7:
